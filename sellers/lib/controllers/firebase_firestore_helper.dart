  ...
  Future<void> updateproduct(ProductModel productModel) async {
    try {
      DocumentReference productRef =
          _firebaseFirestore.collection('products').doc(productModel.productId);
...
//----------------------------------------------------------------
  Future<ProductModel> addProduct(
    ...
    ProductModel addProduct = ProductModel(
      ...
      discount: double.parse(discount).toDouble(),
  ...
  Future<List<OrderModel>> getCompletedOrderList() async {
    try {
      QuerySnapshot<Map<String, dynamic>> collectionCheck =
          await _firebaseFirestore.collection('orderssss').limit(1).get();
